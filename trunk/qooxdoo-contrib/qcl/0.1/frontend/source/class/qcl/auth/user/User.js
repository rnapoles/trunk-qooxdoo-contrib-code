 ...
 * A user object
 */
qx.Class.define("qcl.auth.user.User",
{
  ...
  */
  ...
  construct : function(vName)
  {
    ...
    this.setNamedId(vName);
		this.__roles = {};
		this._manager = qcl.auth.user.Manager.getInstance();
  ...
  members :
  {
     ...
     */
		setRoles : function(roles)
		{
			...
			{
				this.error("Argument must be an array of string values or qcl.auth.role.Role objects.");
			}
     ...
     *
     * @param roleRef {String|qcl.auth.role.Role} name of role object or object reference
    ...
    {
			var name = qcl.auth.role.Manager.getInstance().getNamedId(roleRef); 
     ...
     *
     * @param permissionRef {String|qcl.auth.permission.Permission} name of permission object or object reference
    ...
    hasPermission : function(permissionRef)
    {
		 ...
		 * @return {Map} Hashmap of qcl.auth.permission.Permission objects, key is name
		 */
