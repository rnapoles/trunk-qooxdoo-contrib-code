...
qx.Class.define('qcl.ApplicationEventHandlers',
{
   ...
   */
    authorization_widget_left_03_on_dragdrop: function(event, target)
    {
      ...
      if (target.getNodeType(targetNode) != "qcl.auth.types.Role") 
        return;
   ...
   */
    authorization_widget_left_06_on_qcl_auth_commands_item_delete: function(message, target)
    {
      ...
      var node = target.getApplication().getUserData("currentNode"), id = node.data.id, namedId = node.data.namedId, name = node.data.name, nodeType = target.getNodeType(node), parentNode = target.nodeGet(node.parentNodeId), parentId = parentNode.data ? parentNode.data.id : null, displayName = " '" + (name ? name : namedId) + "'";
      
      if (!confirm("Do you really want to delete " + displayName + "?")) 
        return;
   ...
   */
    authorization_widget_left_07_on_qcl_auth_commands_item_remove: function(message, target)
    {
      ...
      var node = target.getApplication().getUserData("currentNode"), id = node.data.id, nodeType = target.getNodeType(node), shortType = nodeType.substr(nodeType.lastIndexOf(".") + 1), parentNode = target.nodeGet(node.parentNodeId), roleId = parentNode.data.id;
   ...
   */
    ...
    {
      if (!confirm(target.tr("Do you really want to completely delete the selected keys?"))) 
        return;
