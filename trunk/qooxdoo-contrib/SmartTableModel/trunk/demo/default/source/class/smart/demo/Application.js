...
qx.Class.define("smart.demo.Application", {
	...
	members: {
	    ...
	    //
	    // Define our views. These are subsets of the table rows defined in terms of filter functions.
	    // Each view can have a single filter function or a list of functions conjoined by either 
	    // 'and' or 'or' operators.
	    //
	    views: {
		"All Orders": {
		...
		//
		// Every row will have a unique Order Number so we'll use that column as an index.
		// The index will allow us to instantly find any order in the table using its order
		// number.
		//
		tm.addIndex(this.columns["Order Number"]);

		//
		// Add additional views (the unfiltered view is always present, as view zero).
		//
		var id = 0;
		...
		//
		// Enable indexed selection by Order Number. This will cause the model to
		// automatically preserve the selection across table modifications, using the Order
		// Number index.
		//
		...
		tcm.setDataCellRenderer(this.columns["Processed?"], new qx.ui.table.cellrenderer.Boolean());
		tcm.setDataCellRenderer(this.columns["Shipped?"], new qx.ui.table.cellrenderer.Boolean());
		tcm.setDataCellRenderer(this.columns["Order Date"], new qx.ui.table.cellrenderer.Date());
		...
		//
		// Create a view control so the user can select which view to, er..., view.
		//
		var id = 0;
		...
		var This = this;
		setInterval(function () { This.addOrders(Math.random() * 4, false); }, 5*1000);
	    ...
	    firstNames: [
		"Jacob","Michael","Ethan","Joshua","Daniel","Alexander","Anthony","William","Christopher",
		"Matthew","Jayden","Andrew","Joseph","David","Noah","Aiden","James","Ryan","Logan",
		"John","Emma","Isabella","Emily","Madison","Ava","Olivia","Sophia","Abigail","Elizabeth",
	    ...
	    companies: [
		"Acme", "Mainway Industries", "First Rate", "GloboChem", "BigCorp", "AAA", "Tools 'R' Us"
	    ],
	    products: [
		"Sprocket", "Bag o' Glass", "Thumb Drive", "Unicycle", "JavaScript Framework"
	    ],
		...
		for (var i = 0; i < N; i++) {
		    ...
		    toAdd.push([++this.orders,						// Order Number
		...
		//
		// Actually add the rows to the table model. It is more efficient to add many rows
		// at once than to add rows one at a time.
		//
		tm.addRows(toAdd);
	    },
		...
		//
		// See what row the order to be canceled is in. The indexing makes this is very
		// fast. The row number returned is relative to the current view.
		//
		var tm = this.table.getTableModel();
		...
		//
		// Remove the row corresponding to the order to be cancelled.  Note that removing a
