 ...
 */
qx.Class.define("smart.Model", {
	extend : qx.ui.table.model.Simple,

	// Mixins:
	include : smart.UnitTests,	// unit testing code
	...
	members: {
	    ...
	    */
		...
		this.__selectionModel.iterateSelection(function(row) {
			var id = this.getValue(this.__selectionIndex, row);
			if (skip && id in skip)
			    return;
			selected.push(id);
		    },
		...
		if (rowIndex < 0 || rowIndex >= rows)
		    throw new Error("rowIndex out of bounds: " + rowIndex + " (0.." + rows + ")");
		...
		for (var v = (skipviewzero ? 1 : 0); v < this.__views; v++) {
		    ...
		    var was_filtered = (v && (this.__getRowIndex(v, R) == undefined)) ? true : false;

		    // Is it filtered out of this view now?
		    var now_filtered = (v && this.__testAllFilters(v, R)) ? true : false;
	    ...
	    //
	    // Test whether a row or set or rows would be filtered out of a particular view.
	    //
	    // If single is true, then R must be a single row, and this returns true (filtered out)
	    // or false (not filtered out).
	    //
		...
		//
		// If there is zero or one filter function, we can return immediately.
		//
		if (single && flen == 1)
		    return filters[0](R);
		if (flen == 0)
		    return single ? false : R;
		...
		function _test(rowdata) {
		    if (conjunction === 'or') {
			// If any filter returns true, hide the row
			...
			// If all filters return true, hide the row
			for (var i = 0; i < flen; i++)
			    if (!filters[i](rowdata))
				return false;
			return true;
	    ...
	    /*
	     * Binary search using three-way comparator. Returns the index of element e if it's in the array
	     * A. If e is not in A, return the negated index of the element that e would immediately
	     * precede, minus one. (I.e., set insertion_point = -returnvalue - 1) The return value is defined
	     * to avoid ambiguity of the 0 return value.
	     *
