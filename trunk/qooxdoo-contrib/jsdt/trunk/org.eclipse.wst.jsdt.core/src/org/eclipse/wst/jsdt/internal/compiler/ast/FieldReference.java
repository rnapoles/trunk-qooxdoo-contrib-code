...
public class FieldReference extends Reference
  implements InvocationSite, IFieldReference
{
  ...
  // protected FieldBinding codegenBinding; // actual binding used for code
  // generation (if no synthetic accessor)
  // public FunctionBinding[] syntheticAccessors; // [0]=read accessor [1]=write
  // accessor
                                     ...
                                     boolean isCompound )
  {
    ...
    // currentScope.problemReporter().uninitializedBlankFinalField(binding,
    // this);
    ...
    if( receiver instanceof SingleNameReference
        && ( ( SingleNameReference )receiver ).binding instanceof LocalVariableBinding )
    {
    ...
    flowInfo = receiver.analyseCode( currentScope,
                                     flowContext,
                                     flowInfo,
                                     binding == null || !binding.isStatic() )
      .unconditionalInits();
    ...
    if( binding != null && binding.isFinal() ) {
      ...
      if( binding.isBlankFinal()
          ...
          && ( ( receiver.bits & ParenthesizedMASK ) == 0 ) // (this).x is
                                                            // forbidden
          && currentScope.allowBlankFinalFieldAssignment( binding ) )
      {
        if( flowInfo.isPotentiallyAssigned( binding ) ) {
          currentScope.problemReporter()
            .duplicateInitializationOfBlankFinalField( binding, this );
        } else {
          flowContext.recordSettingFinal( binding, this, flowInfo );
        }
        flowInfo.markAsDefinitelyAssigned( binding );
      } else {
        // assigning a final field outside an initializer or constructor or
        // wrong reference
  ...
  /**
   * @see org.eclipse.wst.jsdt.internal.compiler.ast.Expression#computeConversion(org.eclipse.wst.jsdt.internal.compiler.lookup.Scope,
                                 ...
                                 TypeBinding compileTimeType )
  {
    if( runtimeTimeType == null || compileTimeType == null )
      return;
    // set the generic cast after the fact, once the type expectation is fully
    // known (no need for strict cast)
    if( this.binding != null && this.binding.isValidBinding() ) {
      ...
      {
        // TypeBinding targetType = (!compileTimeType.isBaseType() &&
        // runtimeTimeType.isBaseType())
                                                ...
                                                boolean isReadAccess )
  {
    if( ( flowInfo.tagBits & FlowInfo.UNREACHABLE ) != 0 )
      return;
    // if field from parameterized type got found, use the original field at
    // codegen time
    // this.codegenBinding = this.binding.original();
    //
    // if (binding.isPrivate()) {
    // if ((currentScope.enclosingSourceType() !=
    // this.codegenBinding.declaringClass)
    // && binding.constant() == Constant.NotAConstant) {
    // if (syntheticAccessors == null)
    // syntheticAccessors = new FunctionBinding[2];
    // syntheticAccessors[isReadAccess ? READ : WRITE] =
    // ((SourceTypeBinding)
    // this.codegenBinding.declaringClass).addSyntheticMethod(this.codegenBinding,
    // isReadAccess);
    // currentScope.problemReporter().needToEmulateFieldAccess(this.codegenBinding,
    // this, isReadAccess);
    // return;
    // }
    //
    ...
    // syntheticAccessors = new FunctionBinding[2];
    // syntheticAccessors[isReadAccess ? READ : WRITE] =
    // destinationType.addSyntheticMethod(this.codegenBinding, isReadAccess);
    // currentScope.problemReporter().needToEmulateFieldAccess(this.codegenBinding,
    // this, isReadAccess);
    // return;
    //
    ...
    // syntheticAccessors = new FunctionBinding[2];
    // syntheticAccessors[isReadAccess ? READ : WRITE] =
    // currentCompatibleType.addSyntheticMethod(this.codegenBinding,
    // isReadAccess);
    // currentScope.problemReporter().needToEmulateFieldAccess(this.codegenBinding,
    // this, isReadAccess);
    // return;
    ...
    // for runtime compatibility on 1.2 VMs : change the declaring class of the
    // binding
    ...
    // && this.binding.declaringClass.id != T_JavaLangObject) // no change for
    // Object fields
                                  ...
                                  TypeBinding useType )
  {
    ...
    // }
    /*
     * Handle if this is a reference to the prototype of a type By default, the
     * prototype is of type Object, but if there is an InferredType for the
     * receiver, it should yeild the receiver type.
     */
    if( this.isPrototype() ) {
      ...
      if( possibleTypeName != null ) {
        Binding possibleTypeBinding = scope.getBinding( possibleTypeName,
                                                        Binding.TYPE
                                                            & RestrictiveFlagMASK,
                                                        this,
                                                        true /* resolve */);
    ...
    if( this.receiverType == null ) {
      ...
      if( possibleTypeName != null )
        possibleTypeBinding = scope.getBinding( possibleTypeName,
                                                Binding.TYPE
                                                    & RestrictiveFlagMASK,
                                                this,
                                                true /* resolve */);
      if( possibleTypeBinding != null && possibleTypeBinding.isValidBinding() )
      {
        ...
        this.binding = new ProblemFieldBinding( null,
                                                this.token,
    ...
    // }
    // }
    // the case receiverType.isArrayType and token = 'length' is handled by the
    // scope API
    /*
     * Need to look in the fields and method for a match... In JS there is no
     * distinction between member functions or field. We are trying to mimic
     * that property below (Java does have a distinction)
     */
    ...
    if( !memberBinding.isValidBinding()
        && ( this.receiverType != null && this.receiverType.isFunctionType() ) )
    {
    ...
    if( memberBinding instanceof FieldBinding ) {
      ...
      if( JavaScriptCore.IS_ECMASCRIPT4 ) {
        ...
        if( receiverErasure instanceof ReferenceBinding ) {
          ...
          {
            this.receiverType = fieldBinding.declaringClass; // handle indirect
                                                              // inheritance
                                                              // thru variable
                                                              // secondary bound
          }
      ...
      this.receiver.computeConversion( scope,
                                       this.receiverType,
                                       this.receiverType );
      if( isFieldUseDeprecated( fieldBinding,
                                scope,
      ...
      constant = isImplicitThisRcv
                                  ? fieldBinding.constant()
                                  : Constant.NotAConstant;
      if( fieldBinding.isStatic() ) {
        // static field accessed through receiver? legal but unoptimal (optional
        // warning)
        if( !( isImplicitThisRcv || ( receiver instanceof NameReference && receiverIsType ) ) )
        {
      ...
      return this.resolvedType = ( ( ( this.bits & IsStrictlyAssigned ) == 0 )
                                                                              ? fieldBinding.type.capture( scope,
                                                                                                           this.sourceEnd )
                                                                              : fieldBinding.type );
  ...
  public TypeBinding resolveForAllocation( BlockScope scope, ASTNode location )
  {
    ...
    if( typeBinding == null ) {
      if( qualifiedName == null )
        qualifiedName = new char[][]{
          token
        };
      typeBinding = new ProblemReferenceBinding( qualifiedName,
                                                 null,
                                                 ProblemReasons.NotFound );
    }
    return typeBinding;
