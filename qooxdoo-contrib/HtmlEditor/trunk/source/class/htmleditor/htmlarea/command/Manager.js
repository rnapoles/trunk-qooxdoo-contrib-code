 ...
 *
 */
qx.Class.define("htmleditor.htmlarea.command.Manager",
{
  ...
  members :
  {
    ...
    __populateCommandList : function()
    {
      this.commands = {
        ...
        paste                 : { useBuiltin : false, identifier : "Paste", method : "__paste" },
        ...
        removeformat          : { useBuiltin : false, identifier : "RemoveFormat", method : "__removeFormat" },

        setparastyle          : { useBuiltin : false, identifier : "SetParaStyle", method : "__setParaStyle" },
...
//        }
    ...
    {
      var focusNode = this.__editorInstance.getSelection().focusNode;
    ...
    */
    ...
    __insertHtml : qx.core.Variant.select("qx.client",
    {
      ...
      {
        this.__editorInstance.getBody().focus();

        /* Special handling if a "br" element should be inserted */
        if (value == htmleditor.htmlarea.HtmlArea.simpleLinebreak)
        {
    ...
    __removeFormat : function(value, commandObject) {
		var node = this.__editorInstance.getBody();
		
    	 /*
         var node = this.getContentBody();
	...
	/**
	 * Sets the paragraph style of the currently selected paragraph
	 * @param value tagName
	 * @param commandObject
	 */
	__setParaStyle : function(value, commandObject) {
		if (!value)
			return;
        ...
        // We want to convert the entire block into a new style so we must find the block;
        //  starting at the node that the range points to, go up the tree until we find
        //  the block to be replaced.
        // Note that we look for the startContainer only and convert the ENTIRE block; it
        //  is the user's responsibility to arrange for breaks etc first
        var delta = element;
        var lastElement = null;
        while (delta) {
            if (delta.nodeType == qx.dom.Node.ELEMENT) {
                if (delta.tagName == "BODY") {
                    if (lastElement == null)
                        element = delta.firstChild;
                    else
                        element = lastElement;
                    break;
                } else if (delta.tagName == "P" || (delta.tagName.charAt(0) == "H" && delta.tagName.length == 2)) {
                    element = delta;
                    break;
                }
            } else if (delta.nodeType != qx.dom.Node.TEXT) {
                this.debug("__setParaStyle found an unexpected type of node: " + delta.nodeType + ", " + delta.toString());
                return;
            }
            lastElement = delta;
            delta = delta.parentNode;
        }
        
		if (!element) {
			this.debug("__setParaStyle cannot find an element to apply style to");
			return;
		}
		
		var doc = qx.dom.Node.getDocument(element);
		var newNode = doc.createElement(tagName);
		if (element.nodeType == qx.dom.Node.TEXT) {
			newNode.appendChild(doc.createTextNode(element.nodeValue));
		} else {
		...
		qx.dom.Element.replaceChild(newNode, element);
	},
	
	
	/**
	 * Fixes up the HTML, by ensuring that all top-level text are in paragraphs and by trying to remove
	 * unnecessary/empty tags
	 */
	fixupHtml : function() {
    ...
    */
    insertParagraphOnLinebreak : qx.core.Variant.select("qx.client",
    {
     ...
     __insertBrOnLinebreak : qx.core.Variant.select("qx.client", 
     {
       "mshtml" : function()
       {
         ...
         if (parentElement != "li")
         {
           rng.pasteHTML(htmleditor.htmlarea.HtmlArea.simpleLinebreak);
           rng.collapse(false);
           rng.select();
     ...
     __setInOutdent : qx.core.Variant.select("qx.client",
     {
	  ...
	  /**
	   * Handles pasting into the document
	   */
	  __paste : function(value, command) {
       /* Body element must have focus before executing command */
       this.__editorInstance.getBody().focus();
     ...
     */
      ...
      * 
      * @type member
      * @param attributes {Map containing url and range} url to insert
      * @param commandObject {Object} command object
     ...
     __insertHyperLink : qx.core.Variant.select("qx.client",
     {
       "gecko" : function(attributes, commandObject)
       {
        ...
        var rng = attributes.range;
        var sel = null;
        if (!rng) {
            sel = this.__editorInstance.getSelection();
            rng = this.__editorInstance.createRange(sel);
        }
       ...
       "default" : function(attributes, commandObject)
       {
         return this.__doc.execCommand(commandObject.identifier, false, attributes.url);
       }
     }),
     ...
     getCurrentStyles : function(elem)
     {
	   ...
	   var decoration = qx.core.Variant.isSet("qx.client", "mshtml") ? elem.currentStyle : doc.defaultView.getComputedStyle(elem, null);
       //var decoration = this.__editorInstance.getComputedStyle(elem, null);
     ...
     __getTextDecorations : function(parents)
     {
       ...
       for(var i=0; i<parents.length; i++)
       {
	     ...
	     var parentDecoration = qx.core.Variant.isSet("qx.client", "mshtml") ? elem.currentStyle : elem.ownerDocument.defaultView.getComputedStyle(elem, null);
         //var parentDecoration = this.__editorInstance.getComputedStyle(elem, null);
     ...
     __getBackgroundColor : function(parents)
     {
       ...
       for(var i=0; i<parents.length; i++)
       {
         ...
         /* Retrieve computed style*/
	     parentDecoration = qx.core.Variant.isSet("qx.client", "mshtml") ? elem.currentStyle : elem.ownerDocument.defaultView.getComputedStyle(elem, null);
         //parentDecoration = this.__editorInstance.getComputedStyle(elem, null);
     ...
     __setBackgroundImage : function(value, commandObject)
     {
        ...
        */
       if (repeat != null && htmleditor.htmlarea.command.Manager.__backgroundRepeat.indexOf(repeat) < 0 )
       {
         if (qx.core.Variant.isSet("qx.debug", "on"))
         {
           this.error("The value '" +repeat + "' is not allowed for parameter 'repeat'. Possible values are '" + htmleditor.htmlarea.command.Manager.__backgroundRepeat + "'");
         }
         return false;
       }
       else
        ...
        */
       if (position != null && htmleditor.htmlarea.command.Manager.__backgroundPosition.indexOf('|'+position+'|') < 0)
       {
         if (qx.core.Variant.isSet("qx.debug", "on"))
         {
           this.error("The value '" + position + "' is not allowed for parameter 'position'. Possible values are '" + htmleditor.htmlarea.command.Manager.__backgroundPosition + "'");
         }
         return false;
       }
       else
     ...
     * @return void
     */
    focusAfterExecCommand : qx.core.Variant.select("qx.client", {
      ...
      "mshtml" : function()
      {
        qx.event.Timer.once(function(e)
        {
           ...
           * call visualizeFocus to get the right feedback to the user (editor is active)
           */
          //qx.ui.core.ClientDocument.getInstance().setActiveChild(this.__editorInstance);
		  this.__editorInstance.activate();
		  //this.__editorInstance.focus();
          ...
          {
            this.debug ("can't select range in focusAfterExecCommand.");
          }

          this.__editorInstance.visualizeFocus();
        }, this, 50);
      },
  ...
  destruct : function()
  {
    this._disposeFields("__doc", "__editorInstance", "commands");
  }
});
