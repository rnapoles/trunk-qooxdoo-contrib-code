...
class TemplateProvider(object):
    ...
    TEMPLATES_MODULE_DIR_NAME = os.path.join(TEMPLATES_DIR_NAME, "tmp", "makomodule")
    ...
    def __init__(self, dialectSettings):
        ...
        moduleTmplDirName = self.__dialectSettings__.getPathForDialect(self.TEMPLATES_MODULE_DIR_NAME)
        
        logd.debug("Picking up templates by path %s." %dialectTmplDirName)
        logd.debug("Processed templates files will be stored by path %s." %moduleTmplDirName)
        
        # creating mako template lookup
        self.__lookup__ = TemplateLookup(directories=[dialectTmplDirName], module_directory=moduleTmplDirName)
