...
def __getProcessor__(dialect):
    """docstring for __getProcessor__"""
    log.debug("Getting processor for dialect %s." %dialect)
    
    return processor.Processor(dialect)
...
def __processDir__(dirPath, options):
    ...
    if os.path.isdir(dirPath):
        ...
        projectDir = gs.props["projectDir"]
...
def main():
    ...
    gs = settings.GlobalSettings()
    gs.props["projectDir"] = projectSourceDir
    gs.props["cwd"] = os.getcwd()
    ...
    opt.add_option("-s","--dialect",dest="dialect", action="store", default="qxml")
...
def getProjectPath(path):
    ...
    gs = settings.GlobalSettings()
    projectSourceDir = gs.props["projectDir"]
