...
Thiscontribution is a scriptable test client for your JSON-RPC backend. 
...
demo application (rpcconsole.Application), which can be used run tests
manually or by user-supplied scripts. 

The application allows to open multiple windows with independent consoles, and to 
...
===== Note on the json-rpc version =====

The current json-rpc implementation in qooxdoo is differs from the current [[http://json-rpc.org/wiki/specification|json-rpc 1.0 specification]] in several respects. For example, 1.0 specs do not use the "service" and "server_data" parameters, and do not support a native javascript date object. It is planned to implement a new RPC layer that is standard-compliant and will also support [[http://groups.google.com/group/json-rpc/web/json-rpc-2-0|json-rpc version 2.0]]. When that happens, the console will be updated to support version 2.0. However, it will always stay backward-compatible and support the current implementation.

At the moment, you can control the version by selecting "non-standard" and "1.0" from a select box in the console. You can also set a version by providing a "mode" URL GET parameter when loading the application.
...
When running a test suite from test data (see below), the result of the tests will be reported in a table widget. You will see icons indicating success or failure of the test, and after all tests have finished, you can click on individual rows to 
