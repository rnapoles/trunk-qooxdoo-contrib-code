 ...
 * A permission object
 */
qx.Class.define("qcl.auth.role.Role",
{
  ...
  */
  ...
  construct : function(vName)
  {
    ...
    this.setNamedId(vName);
		this.__permissions = {};
		this._manager = qcl.auth.role.Manager.getInstance(); 
  ...
  members :
  {
     ...
     */
		setPermissions : function(permissions)
		{
			...
			{
				this.error("Argument must be an array of string values or qcl.auth.permission.Permission objects.");
			}
     ...
     * permission reference can be a qcl.auth.permission.Permission object or a string with
      ...
      else if ( name )
      {
        ...
        return true;
			...
			else if ( typeof permissionRef == "string" && permissionRef.indexOf("*") > -1 )
			{
				...
				permissions[permissionRef] = new qcl.auth.permission.Permission(permissionRef);			
			}
			else
			{
