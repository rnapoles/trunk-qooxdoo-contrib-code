...
@author sbull
*/
qx.OO.defineClass("ext.strategy.position.GridStrategy",
                  ext.strategy.position.AbstractPositionStrategy,
function(vWorkspace, vOrigin, vDirection, vWrap) {
  ...
  this._supportedOrigins = [ext.constant.Position.TOP_LEFT,
                            ext.constant.Position.TOP_RIGHT,
                            ext.constant.Position.BOTTOM_LEFT,
                            ext.constant.Position.BOTTOM_RIGHT];
  ...
  this._supportedDirections = [ext.constant.Direction.HORIZONTAL_LEFT,
                               ext.constant.Direction.HORIZONTAL_RIGHT,
                               ext.constant.Direction.VERTICAL_UP,
                               ext.constant.Direction.VERTICAL_DOWN];
  ...
  this._origin = (this.isValidOrigin(vOrigin))
               ? vOrigin
               : ext.constant.Position.BOTTOM_LEFT;
  ...
  this._direction = (this.isValidDirection(vDirection))
                  ? vDirection
                  : ext.constant.Position.HORIZONAL_RIGHT;
  ...
  this._wrap = (this.isValidDirection(vWrap))
             ? vWrap
             : ext.constant.Direction.VERTICAL_UP;
...
*/
qx.OO.addProperty({ name : "iconSize", type : "number", defaultValue : 48, allowNull : false});
...
/*
  ...
  For internal use only.
*/
qx.OO.addProperty({ name : "gridCellSize", type : "number", defaultValue : 84, allowNull : false});
...
/*
  ...
  For internal use only.
*/
qx.OO.addProperty({ name : "gridCellSpacing", type : "number", defaultValue : 6, allowNull : false});
...
/*
  ...
  For internal use only.
*/
qx.OO.addProperty({ name : "gridMargin", type : "number", defaultValue : 6, allowNull : false});
...
qx.Proto.getNumberOfRows = function()
{
  return this._numberOfRows;
}


qx.Proto.getNumberOfCols = function()
{
...
*/
qx.Proto._repositionAll = function()
{
  this._initGrid();
...
qx.Proto._positionOLD = function(vWidget, index)
{
  // ************************************************************************
  //   GRIDCELL COORD
  // ************************************************************************
...
/*
*/
qx.Proto._position = function(vWidget, index)
{
  ...
  if (   this._origin == ext.constant.Position.BOTTOM_LEFT
      || this._origin == ext.constant.Position.BOTTOM_RIGHT)
  {
    baseTop += this._verticalPadding;
  }

  var baseLeft = this._gridLeft + cellSpacing;
  if (   this._origin == ext.constant.Position.TOP_RIGHT
      || this._origin == ext.constant.Position.BOTTOM_RIGHT)
  {
...
*/
qx.Proto._getNextAvailableIndex = function()
{
  var all = this.getAllWidgets();
  var index = 0;
...
*/
qx.Proto._orderGridCells = function()
{
  this._coords = [];
...
*/
qx.Proto._renderGridCells = function()
{
  if (this._debug)
  {
    this._orderGridCells();
...
qx.Proto._getNextGridCoord = function(nextTop, nextLeft, minTop, minLeft, maxTop, maxLeft)
{
...
qx.Proto._getCellObj = function(top, left)
{
  var obj = {};
      obj.top = top;
      obj.left = left;
...
*/
qx.Proto._setDimensions = function()
{
...
/*
  ...
  been executed first.
*/
qx.Proto._computeMarginBounds = function()
{
  this.computeBounds();
